      ...
      <el-table ref="productCateTable"
        ...
        <el-table-column label="设置" width="200" align="center">
          <template slot-scope="scope">
            <el-button
              size="mini"
              @click="handleShowNextLevel(scope.$index, scope.row)">{{ label }}
            </el-button>
            <!-- <el-button
              size="mini"
  ...
  export default {
    ...
    methods: {
      ...
      handleShowNextLevel(index, row) {
        ...
        console.log(level);
        if (level == 0) {
          this.$router.push({path: '/pms/productCate', query: {parentId: row.id, level: level}})
        } else if (level == 1) {
      ...
      handleDelete(index, row) {
        this.$confirm('是否要删除该品牌', '提示', {
          ...
          deleteProductCate(row.id).then(response => {
            ...
            // 计算被删除元素是否为最后一页的第一个元素
            // 是：页码往前一页
            // 否：不做更改
            let afterTotal = this.total - 1;
            let res = afterTotal % this.listQuery.pageSize
            if (res == 0 && this.listQuery.pageNum != 1) {
